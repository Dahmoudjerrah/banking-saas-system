...
class TenantRouter:
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        ...
        # Empêche les migrations des applications 'transactions' et 'accounts' dans la base de données 'default'
        # Ces apps contiennent maintenant les nouveaux modèles spécialisés (PersonalAccount, BusinessAccount, etc.)
        if app_label in ['transactions', 'accounts']:
            return db != 'default'
        
        # Pour les nouvelles tables spécialisées, s'assurer qu'elles sont créées sur les bonnes bases
        if model_name in [
            'personalaccount', 
            'businessaccount', 
            'agencyaccount', 
            ...
            # Ces modèles ne doivent PAS être migrés sur la base 'default'
            return db != 'default'
